...
- Parameter(optional): **cookie** - Cookie value to use in the authentication process. This can be used to provide a truly random cookie for enhanced security. If this parameter is not specified then a pseudorandom cookie will be generated using `random()`.
  - Type: long
- Parameter(optional): **password** - Password to use for receiving the event. If the password parameter is used then the cookie parameter must be specified (if you want EtherEvent to generate your cookie then use `false` for the cookie parameter).  To disable authentication on a per-receive basis use an empty password (`""`).
...
Puts the event in the passed array. Call `EtherEvent.availableEvent()`, size a char array according to the result, and pass it to `EtherEvent.readEvent()`. After that the char array will contain the event.
...
Puts the payload string in the passed array. Call `EtherEvent.availablePayload()`, size a char array according to the result, and pass it to `EtherEvent.readPayload()`. After that the char array will contain the payload.
...
##### `EtherEvent.senderIP()`
Returns the IP address of the sender of the most recent event. You must have the [modified Ethernet library](http://github.com/per1234/EthernetMod) installed for this function to be available.
...
- Parameter: **ethernetClient** - The EthernetClient object created in the Ethernet setup of your sketch.
  - Type: EthernetClient
...
- Parameter: **port** - Port to send the event to.
  - Type: unsigned int
- Parameter: **event**
  - Type: char array, int8_t, byte, char, int, unsigned int, long, unsigned long, __FlashStringHelper (`F()` macro), String, IPAddress, float, double, Printable (not supported in FAST_SEND mode)
...
Set the timeout duration (used by Stream functions).
...
- Debug output: By turning debug output on you can get details of the authentication process in the Serial Monitor. Add the line `#define ETHEREVENT_DEBUG true` to your sketch before the line `#include <EtherEvent.h>`, this will slow down communication and increase memory usage so only enable debug output when needed. The default speed of the debug serial output is 9600 but this can be changed by defining `ETHEREVENT_DEBUG_BAUDRATE` in the sketch before the line `#include <EtherEvent.h>.
- If EventGhost receives an event that contains '.' the standard event prefix will disappear from that event. This is a limitation of EventGhost, not EtherEvent.
...
- Cookie Randomization: An important factor in the authentication security is the randomness of the cookie supplied by the event receiver. There is a trade-off between security and memory usage/speed so the ideal level of cookie randomization depends on your application.
  ...
  - User supplied cookie: You can generate your own random cookie and pass the value to `EtherEvent.availableEvent()`.
    - Entropy library: A truly random number can be generated by the Entropy library and used for every cookie. This will cause greater flash memory usage and slower authentication. See the example sketch **Entropy.ino** for demonstration of use of the Entropy library with EtherEvent. The Entropy library is compatible with AVR based boards (such as Uno, Mega, Leonardo, etc), Due, and Teensy.
